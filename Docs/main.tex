...
\noindent The file \texttt{models.json} contains a dictionary where the models can be called together with the priors used for their input variables and the order or the inputs to the model in the form of indexes (\texttt{inputs} in the \texttt{json} file dictionary).\\
...
\begin{enumerate}
    ...
    \item Re-run additional points of the optimal log-likelihood algorithm in the area where MCMC drew points.
    \item Start from point 2.
\end{enumerate}

\textbf{Proposed diagnostics.} A tricky issue may arise for non-experts in these analyses. The workflow described cannot be run blindly, without thoroughly asserting the quality of the results. As this requires some in-depth knowledge of the analyses, I propose here a quick way of checking is the GP surrogate + MCMC sampling give reasonable results or if the surrogate estimation would need additional points.
