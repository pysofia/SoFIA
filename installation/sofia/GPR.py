...
class GP:
    ...
    def kernel(self,x,xp,hyp=None):
        ...
        return (self.hyperparams[0]**2)*np.exp(np.divide(-1*np.linalg.norm(x-xp)**2,2*self.hyperparams[1]**2))
    ...
    def log_lik(self,hyp=None):
        ...
        for i in range(self.np):
            for j in range(self.np):
                self.cov[i,j] = self.kernel(self.X[i],self.X[j])

        if i==j:
            self.cov[i,j] += 0.00001
